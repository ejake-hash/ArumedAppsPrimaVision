<script setup>
/**
 * Dokumen cetak Kwitansi/Rincian Biaya (A4) — SUMBER TUNGGAL layout kwitansi kasir.
 * Dipakai oleh KasirView.vue dan LaporanMarketingView.vue agar kwitansi SAMA PERSIS.
 *
 * Render via <Teleport to="body"> + gaya GLOBAL @media print: cukup set prop `data`
 * (payload dari /kasir/invoice/{id}/cetak atau /laporan-marketing/kwitansi/{id})
 * lalu panggil window.print() dari parent. Tidak menampilkan apa pun di layar.
 *
 * @prop data        Payload kwitansi (generateReceipt). null = tidak render.
 * @prop categories  Master billing_categories (untuk urutan grouping). Opsional.
 */
...
// Jenis layanan kunjungan (judul kwitansi + label).
...
// Item dikelompokkan per category, mengikuti sort_order master billingCategories.
// Item yang category-nya tidak terdaftar di master → bucket "Lainnya" di akhir.
...
// (layout "Bruto → Diskon → Harga Paket").
...
const groupedPrintItems = computed(() =>
  groupItemsByCategory(
    ...
    props.data?.categories ?? props.categories,
  ),
)
</script>
