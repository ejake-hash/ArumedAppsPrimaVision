<script setup>
/**
 * ReturBhpIolModal — sisi UNIT BEDAH untuk meretur BHP/IOL ke gudang Inventori Farmasi.
 * Konsep identik dengan ReturObatModal farmasi, tapi station=BEDAH & item BHP+IOL.
 *  - Buat Retur : pilih tipe + item + qty + kondisi + alasan → create + submit (SUBMITTED).
 *  - Histori    : pantau status. Stok kembali ke gudang saat admin receive().
 */
...
const props = defineProps({
  ...
  bhp:  { type: Array,   default: () => [] },
  iol:  { type: Array,   default: () => [] },
  // Prefill retur dari satu permintaan: { unit_request_id, items:[{item_type,item_id,qty}] }
...
function iolLabel(o) {
  return [o.brand, o.model].filter(Boolean).join(' ') + (o.power ? ` · ${o.power}D` : '')
}
function optionsFor(type) { return type === 'IOL' ? props.iol : props.bhp }
function unitOf(row) {
  if (!row.item_id) return ''
  if (row.item_type === 'IOL') return 'Pcs'
  return props.bhp.find((b) => b.id === row.item_id)?.unit ?? ''
}
...
async function fetchList() {
  ...
  try {
    const res = await unitReturnApi.list({ station: 'BEDAH', per_page: 50 })
...
function emptyRow() { return { item_type: 'BHP', item_id: '', qty: 1, condition: 'GOOD' } }
function addRow() { rows.value.push(emptyRow()) }
function removeRow(i) { rows.value.splice(i, 1); if (!rows.value.length) addRow() }
function onTypeChange(row) { row.item_id = '' }

const unitRequestId = ref(null)   // jika retur ditautkan ke satu permintaan
...
async function saveReturn() {
  ...
  if (!items.length) {
    emit('changed', { type: 'w', message: 'Pilih minimal 1 item dengan qty > 0' }); return
  }
  ...
  saving.value = true
  try {
    const payload = { returning_station: 'BEDAH', reason: reason.value.trim(), items }
    if (unitRequestId.value) payload.unit_request_id = unitRequestId.value
    const res = await unitReturnApi.create(payload)
...
watch(() => props.open, (o) => {
  if (!o) return
  reason.value = ''
  unitRequestId.value = props.prefill?.unit_request_id ?? null
  const pf = props.prefill?.items
  if (Array.isArray(pf) && pf.length) {
    rows.value = pf.map((it) => ({
      item_type: it.item_type ?? 'BHP',
      item_id:   it.item_id ?? '',
    ...
    <div class="ro-modal">
        ...
        <table class="ro-table">
          ...
          <tbody>
            ...
            <tr v-for="(r, i) in list" :key="r.id">
              ...
              <td class="c"><span class="ro-badge" :class="STATUS[r.status]?.cls">{{ STATUS[r.status]?.label ?? r.status }}</span></td>
              <td class="c">
                <div class="ro-acts">
      ...
      <div v-else class="ro-body">
        <div class="ro-field">
          ...
          <input v-model="reason" class="ro-input" placeholder="cth: tidak terpakai saat operasi / salah ambil / mendekati kadaluarsa" />
        </div>
        <table class="ro-table">
          <thead>
            <tr>
              ...
              <th>Item</th>
              <th style="width:84px" class="c">Qty</th>
            ...
            <tr v-for="(r, i) in rows" :key="i">
              ...
              <td>
                <select v-model="r.item_id" class="ro-input">
                  <option value="">— pilih {{ r.item_type }} —</option>
                  <option v-for="o in optionsFor(r.item_type)" :key="o.id" :value="o.id">
                    {{ r.item_type === 'IOL' ? iolLabel(o) : o.name }}
                  </option>
                </select>
              </td>
