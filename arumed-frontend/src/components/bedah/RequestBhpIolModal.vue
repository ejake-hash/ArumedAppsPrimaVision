<script setup>
/**
 * RequestBhpIolModal — sisi UNIT BEDAH untuk meminta BHP/IOL ke gudang Inventori Farmasi.
 * Konsep identik dengan RequestObatModal farmasi, tapi station=BEDAH & item BHP+IOL.
 *  - Buat Permintaan : pilih tipe + item + qty → create + submit (status SUBMITTED).
...
const props = defineProps({
  ...
  bhp:  { type: Array,   default: () => [] },   // master BHP  [{id, name, code, unit}]
  iol:  { type: Array,   default: () => [] },   // master IOL  [{id, brand, model, power}]
})
...
async function fetchList() {
  ...
  try {
    const res = await unitRequestApi.list({ station: 'BEDAH', per_page: 50 })
...
const terimaReq = (row) => act(row, () => unitRequestApi.close(row.id), `Barang ${row.request_number} diterima`)
...
function emptyRow() { return { item_type: 'BHP', item_id: '', qty: 1 } }
...
async function saveRequest() {
  ...
  saving.value = true
  try {
    const res = await unitRequestApi.create({ requesting_station: 'BEDAH', items })
      ...
      <div v-else class="ro-body">
        <table class="ro-table">
          <thead>
            <tr>
              <th style="width:44px">No.</th>
              <th style="width:96px">Tipe</th>
              <th>Item</th>
              <th style="width:110px" class="c">Qty</th>
